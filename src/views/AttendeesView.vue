  ...
  <div class="min-h-screen bg-gray-50 py-8">
    <div class="container mx-auto px-4">
      ...
      <div class="text-center mb-8">
        <h1 class="text-4xl font-bold text-gray-900 mb-2">Angemeldete Teilnehmer</h1>
        ...
        <div class="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-green-100 text-green-800 rounded-full">
          ...
          <span class="font-semibold">{{ confirmedParticipants.length + reservedParticipants.length }} von 8 Plätzen belegt</span>
        </div>
      </div>

      <!-- Gesamte Übersicht in einer Tabelle -->
      <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="bg-gradient-to-r from-green-500 to-blue-500 text-white px-6 py-4">
          <h2 class="text-xl font-bold flex items-center gap-2">
            <i class="pi pi-list"></i>
            Alle Startplätze ({{ totalSlots }})
          </h2>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr v-for="participant in confirmedParticipants" :key="participant.id" class="hover:bg-green-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span v-if="participant.lookingForTeammate" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
              ...
              <tr v-for="participant in reservedParticipants" :key="participant.id" class="hover:bg-orange-50">
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
              ...
              <tr v-for="slot in freeSlotNumbers" :key="slot" class="hover:bg-blue-50">
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <div class="w-8 h-8 bg-gray-300 text-gray-600 rounded-full flex items-center justify-center font-bold">
                      {{ slot }}
                    </div>
                  </div>
                </td>
      ...
      <div v-if="freeSlots === 0" class="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
        ...
        <p class="text-red-600">Alle 8 Startplätze sind vergeben. Schauen Sie gerne bei unserem nächsten Turnier vorbei.</p>
      </div>
    </div>
  </div>
</template>
...
const totalSlots = 8
const freeSlots = computed(() => totalSlots - confirmedParticipants.value.length - reservedParticipants.value.length)
